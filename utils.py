...
import pandas as pd
import requests
evaluationId = "a387eb2c-a286-42da-887d-836cfaee7bc2"
sessionId = "IskdLW1NM08ZMZ14O0dc3kgxBM7tRfAH"
...
def submit_tool(image_path, type):
                  ...
                  "answerSets": [
                      {
                          "answers": [
                              {
                                  "text": f"{ans}-{video_id}-{result_timer}"
                              }
                          ]
                      }
                  ]
    ...
    'Content-Type': 'application/json'
      ...
      # response = reque  sts.post(url, headers=headers, data=json.dumps(data))
      # if response.status_code == 200:
      #     print("Submission successful!")
      # else:
      #     print(f"Failed to submit: {response.status_code}")
